# Script para restaurar main_window.py
import os

# Conteúdo correto do main_window.py
content = '''# src/main_window.py
...
class PaginaBoasVindas(QWidget):
    def __init__(self):
        ...
        # Container central com estilo
        container = QWidget()
        container.setObjectName("welcome_container")
        container_layout = QVBoxLayout(container)
        container_layout.setSpacing(20)
        
        logo_label = QLabel("Audit+ v2.0")
        logo_label.setObjectName("logo_label")
        logo_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        titulo = QLabel("Bem-vindo ao Audit+")
        titulo.setObjectName("titulo_pagina")
        titulo.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        subtitulo = QLabel("Sua ferramenta de automação e validação de arquivos PTU.")
        ...
        info_label = QLabel(
            ...
            "✓ Distribuição inteligente por auditor\\n"
            "✓ Validação de regras TISS\\n"
            "✓ Atualização de hash simplificada"
        )
        info_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        info_label.setStyleSheet("color: #D8DEE9; font-size: 14px; line-height: 1.8;")
...
class PaginaHash(QWidget):
    def __init__(self, controller):
        ...
        info_label = QLabel(
            "Selecione o auditor para processar os arquivos da pasta 'Correção XML' e recriar os ZIPs finais."
        )
        info_label.setWordWrap(True)
        layout.addWidget(info_label)

        self.btn_atualizar_hash = QPushButton("# Atualizar Hash e Recriar ZIPs")
        self.btn_atualizar_hash.setMinimumHeight(45)
        self.btn_atualizar_hash.setToolTip("Atualiza hash dos XMLs e recria arquivos ZIP")
    ...
    def iniciar_atualizacao_hash(self):
        ...
        if ok and nome_auditor:
            ...
            self.worker_thread = QThread()
            self.worker = Worker(self.controller.executar_atualizacao_hash, nome_auditor)
            self.worker.moveToThread(self.worker_thread)
...
# Escrever o arquivo
target_path = r'c:\Users\pedro.freitas\AuditPlusv2.0\src\main_window.py'
with open(target_path, 'w', encoding='utf-8') as f:
    f.write(content)

print(f"Arquivo restaurado com sucesso: {target_path}")
